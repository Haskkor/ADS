...
fondecran = pygame.image.load("fond_gris.jpg")
maSurface2.blit(fondecran,(0,0))
...
def tourdesblancs():
    maSurface2.blit(tourblanc,(150,550))
def tourdesnoirs():
...
def recommencer():
    ...
    ligne=4
    global nbrdedpion
    nbrdepion=marshal.load(open("1snbrdepion", "rb"))
    global rotation
    global plateau1
    global plateau2
    global plateau3
...
#    plateau=[]
    for i in range(ligne):
        ...
        for j in range(colonne):
            plateau[i].append(2)
    return plateau
def tableaublanc(plateau,ligne):
    colonne=ligne
#    plateau=[]
    for i in range(ligne):
        plateau.append([])
...
ligne=4
rotation=[]
plateau1=marshal.load(open("1splateau1", "rb"))
plateau2=marshal.load(open("1splateau2", "rb"))
plateau3= marshal.load(open("1splateau3", "rb"))
plateau4= marshal.load(open("1splateau4", "rb"))
...
def animvictoire():
    ...
    for i in range (1,40):
        ...
        texteSurface3 = fontObj.render("VICTOIRE DES",True,NOIR)
        texteSurface4 = fontObj.render("BLANCS !",True,NOIR)
...
def animvictoire1():
    ...
    for i in range (1,40):
        ...
        fontObj = pygame.font.Font('freesansbold.ttf',(w))
        texteSurface3 = fontObj.render("VICTOIRE DES",True,BLANC)
        texteSurface4 = fontObj.render("NOIRS !",True,BLANC)
        texteRect = texteSurface3.get_rect()
        texteRect.topleft = (135,220)
        maSurface2.blit(texteSurface3,texteRect)
        texteRect = texteSurface3.get_rect()
...
##1ère case
    ...
    for x in range (4):
        for y in range (4):          
            pygame.draw.circle(maSurface2,BLEUTROU,(posx,posy),14,0)
            posx+=40
        posy+=40
        posx=130
...
def affichepion():
    ...
    for i in range(4):
        for j in range(4):
            if plateau1[j][i] == 0:
                pygame.draw.circle(maSurface2,BLEUTROU,(130+i*40,130+j*40),14,0)
            if plateau1[j][i] == 1:
                pygame.draw.circle(maSurface2,BLANC,(130+i*40,130+j*40),14,0)
            if plateau1 [j][i]==2:
                pygame.draw.circle(maSurface2,NOIR,(130+i*40,130+j*40),14,0)

            if plateau2[j][i] == 0:
                pygame.draw.circle(maSurface2,BLEUTROU,(130+i*40,300+j*40),14,0)
            if plateau2[j][i] == 1:
                pygame.draw.circle(maSurface2,BLANC,(130+i*40,300+j*40),14,0)
            if plateau2 [j][i]==2:
                pygame.draw.circle(maSurface2,NOIR,(130+i*40,300+j*40),14,0)
                

            if plateau3[j][i] == 0:
                pygame.draw.circle(maSurface2,BLEUTROU,(300+i*40,130+j*40),14,0)
            if plateau3[j][i] == 1:
                pygame.draw.circle(maSurface2,BLANC,(300+i*40,130+j*40),14,0)
            if plateau3 [j][i]==2:
                pygame.draw.circle(maSurface2,NOIR,(300+i*40,130+j*40),14,0)

            if plateau4[j][i] == 0:
                pygame.draw.circle(maSurface2,BLEUTROU,(300+i*40,300+j*40),14,0)
            if plateau4[j][i] == 1:
                pygame.draw.circle(maSurface2,BLANC,(300+i*40,300+j*40),14,0)
            if plateau4 [j][i]==2:
                pygame.draw.circle(maSurface2,NOIR,(300+i*40,300+j*40),14,0)
...
def sauvegarde():
    ...
    marshal.dump(cptnoir, open("1scptnoir", 'wb'))
    marshal.dump(cptnoir, open("1scptblanc", 'wb'))
    marshal.dump(plateau1, open("1splateau1", 'wb'))
    marshal.dump(plateau2, open("1splateau2", 'wb'))
    marshal.dump(plateau3, open("1splateau3", 'wb'))
    marshal.dump(plateau4, open("1splateau4", 'wb'))
    marshal.dump(nbrdepion, open("1snbrdepion", 'wb'))
    marshal.dump(joueur, open("1sjoueur", 'wb'))
...
player=2
nbrdepion=marshal.load(open("1snbrdepion", "rb"))
joueur = marshal.load(open("1sjoueur", "rb"))
cptnoir=marshal.load(open("1scptnoir", "rb"))
cptblanc= marshal.load(open("1scptblanc", "rb"))
fin=False
execo=False
a=1
affichepion()
...
while inProgress:
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONDOWN:
            ...
            #matchnul()
        if gagne==False and fin==False and execo==False:
            
            sauvegarde1 ()
        if event.type == MOUSEBUTTONDOWN  and gagne==False and fin==False and execo==False:
            X,Y = event.pos
            ...
            for i in range(4):
                for j in range(4):                                      
                    if plateau1[j][i] == 0:                               
                        if (110+i*40) < X < (110+(i+1)*40) and (110+j*40) < Y < (110+(j+1)*40) and not plateaufull(plateau1):                            
                            if joueur == 1:                                      
                                plateau1[j][i] = 1        
                                affichepion()
                                reuni()
                                ...
                                if fin==False:
                                    ...
                                    gagne=True                                 
                    if plateau2[j][i] == 0:
                        if (110+i*40) < X < (110+(i+1)*40) and (275+j*40) < Y < (275+(j+1)*40) and not plateaufull(plateau2):
                            if joueur == 1:                                
                                plateau2[j][i] = 1
                                affichepion()
                                reuni()
                                ...
                                if fin==False:
                                    ...
                                    gagne=True 
                    if plateau3[j][i]==0:
                        if (275+i*40) < X < (275+(i+1)*40) and (110+j*40) < Y < (110+(j+1)*40) and not plateaufull(plateau3):
                            if joueur == 1:
                                plateau3[j][i] = 1
                                affichepion()
                                reuni()
                                ...
                                if fin==False :
                                    ...
                                    gagne=True 
                    if plateau4[j][i] == 0:
                        if (275+i*40) < X < (275+(i+1)*40) and (275+j*40) < Y < (275+(j+1)*40) and not plateaufull(plateau4):
                            if joueur == 1:
                                plateau4[j][i] = 1                                
                                affichepion()
                                reuni()
